...
private:
    ...
    * The above text was also generated by Claude
    */
    ...
    **************************************************************************/
    ...
    void sparseDirectExchange(
        ...
        if (!toRank[mpiRank].empty()) {
            incomingByRank[mpiRank] = std::move(toRank[mpiRank]);
            toRank[mpiRank].clear();
        }
        ...
        // BUG FIX: outCount must outlive the Isend until Waitall completes.
        // The old code declared `int outCount` as a loop-local variable and
        // passed &outCount to MPI_Isend. The variable was destroyed at the end
        // of each loop iteration, leaving the Isend holding a dangling pointer.
        // Under MadMPI (which does not always eagerly copy small send buffers)
        // this caused the receiver to read garbage -- often 0 -- for the count,
        // which meant no payload Irecv was posted, yet the payload Isend fired
        // anyway, producing the "received more data than expected / expected
        // len = 0" warnings and occasional illegal instruction crashes.
        // Fix: collect all counts into a persistent vector first, then issue
        // Isend calls with stable addresses inside that vector.

        // First pass: collect destination ranks and their counts
        vector<unsigned int> sendDsts;
        vector<int> sendCountBuf;
        sendDsts.reserve(mpiSize);
        sendCountBuf.reserve(mpiSize);
        for (unsigned int dstRank = 0; dstRank < mpiSize; dstRank++) {
            if (dstRank == mpiRank || !willSendToRank[dstRank]) {
                continue;
            }
            sendDsts.push_back(dstRank);
            sendCountBuf.push_back((int)toRank[dstRank].size());
        }

        // Second pass: issue Isend using stable addresses in sendCountBuf
        vector<MPI_Request> countSends;
        countSends.reserve(sendDsts.size());
        for (unsigned int i = 0; i < sendDsts.size(); i++) {
            MPI_Request req;
            MPI_Isend(
                &sendCountBuf[i],   // address stable for lifetime of sendCountBuf
                1,
                MPI_INT,
                (int)sendDsts[i],
                countTag,
